...
static DenseMap<int64_t, OpFoldResult> getDimAndTileMappingImpl(OpTy op) {
  ...
  SmallVector<int64_t> dimsToTile =
...
static bool isInvalid(ArrayRef<int64_t> dimsPos, size_t rank) {
  // early exit.
...
static LogicalResult commonVerifierPackAndUnPackOp(OpTy packOrUnPack) {
  ...
  // Tiling factors must be less or equal than the input rank for pack (or
  // output rank for unpack), and must match the number of `inner_dims_pos`.
  if (mixedTiles.size() > unpackedRank) {
    return op->emitError("tiling factors must be less or equal than the "
...
LogicalResult PackOp::verify() {
  ...
  if (auto paddingValue = getPaddingValue()) {
    if (paddingValue.getType() != getSourceType().getElementType()) {
...
ShapedType PackOp::getPackedType(ShapedType sourceType,
  ...
  resultShape = interchange(resultShape, outerDimsPerm);
