...
static SmallVector<int64_t> computeOuterDims(ArrayRef<int64_t> perm,
  ...
  size_t numberOfExprs = exprs.size();
  for (unsigned pos = 0; pos < numberOfExprs; pos++) {
    unsigned posInDomain = exprs[pos].cast<AffineDimExpr>().getPosition();
...
// Return true if the value has on users.
static bool hasZeroUser(Value blockArg) {
  return std::distance(blockArg.getUsers().begin(),
                       blockArg.getUsers().end()) == 0;
}
...
bubbleUpPackOpThroughElemGenericOp(RewriterBase &rewriter,
  ...
  bool hasNoUser = hasZeroUser(genericOp.getRegionOutputArgs()[0]);
  ...
  Value dest = (hasNoUser) ? packOp.getDest() : packedOutOperand;
